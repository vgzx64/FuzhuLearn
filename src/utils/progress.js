// ponytail: localStorage-backed progress tracker with SM-2 spaced repetition.
...
export function markHanziReviewed(char, score) {
  const p = getProgress();
...
export function markHanziForgotten(char) {
  const p = getProgress();
  p.hanzi[char] = {
    status: 'new',
    correct: 0,
...
export function getNewHanzi(level, hanziList, limit = DECK_CONFIG.newCardsPerDay) {
  const p = getProgress();
...
export function getDueHanzi(level, hanziList, limit = DECK_CONFIG.maxReviewsPerDay) {
  const p = getProgress();
  const chars = hanziList.filter(h => h.l === level);
...
// ponytail: daily new card limit uses a simple localStorage counter. Upgrade: proper settings.
export function getSessionCards(level, hanziList) {
  const p = getProgress();
  ...
  const newCards = getNewHanzi(level, hanziList, Math.min(DECK_CONFIG.maxNewPerSession, remainingNew));
  const dueCards = getDueHanzi(level, hanziList, DECK_CONFIG.maxReviewsPerDay);
